      ...
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          return (0, _axios.default)({
            method: 'PATCH',
            url: url,
            data: data // === data:data === data:{name,email} or data: { passwordCurrent, password, passwordConfirm }
          });
        case 4:
          res = _context.sent;
          //res is obj has the token and user data except password like in login function user
...
function asyncGeneratorStep(gen, resolve, reject, _next, _throw, key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { Promise.resolve(value).then(_next, _throw); } }
function _asyncToGenerator(fn) { return function () { var self = this, args = arguments; return new Promise(function (resolve, reject) { var gen = fn.apply(self, args); function _next(value) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value); } function _throw(err) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err); } _next(undefined); }); }; } /*eslint-disable */
var stripe = Stripe('pk_test_51NlQ3iKJs7hOOE559SmlpM2oxwmWZKN76fnQXmkVxRPl1fgCil1vnW7kuJKGkogS1MZADoQl5amE00su9Q3D6kCR00ed578s08');
...
if (mapSection) {
  var locations = JSON.parse(mapSection.dataset.location);
      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            ...
            passwordConfirm = document.getElementById('password-confirm').value; // here we use await just to wait it finish so we can use the lines under it , cuz updateSetting is async fun and it return a promise that we can await inside async fun (e)
            _context.next = 7;
